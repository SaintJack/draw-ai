 ...
 * 带 ref 的画布组件（用于导出）
 */
export const DrawingCanvasWithRef = forwardRef<View, DrawingCanvasProps>(
  ({onShapePress}, ref) => {
    ...
    const [draggingShapeId, setDraggingShapeId] = React.useState<string | null>(
      null,
    );
    const [dragOffset, setDragOffset] = React.useState<Point>({x: 0, y: 0});

    // 处理图形点击
    const handleShapePress = React.useCallback(
      (shapeId: string) => {
